...
# The QQ plot looks good and the residuals vs. predicted plot looks better than mod\_glm, the dark line is more fitted to the dotted line.

# The residuals vs. explantory variables plots are all in good shapes. The only thing to concern is the plot for “psec\_ssite” is more curved than expected, which

# might indicate some differences between residuals and predicted values for psec\_ssite.
...
# species, but also somehow related to the density of habitat patches. For the tree speices, we can see from the summary of mod\_glmm, koalas really prefer primary tree.
